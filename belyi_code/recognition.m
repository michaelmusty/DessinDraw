...
intrinsic TriangleRescaleCoefficients(Gamma::GrpPSL2, coeffs::SeqEnum, vals::SeqEnum : rescale_ind := 0) -> Any
  ...
  printf "nonzero_vals = %o\n", nonzero_vals;
  print gcd;
  printf "wts = %o\n", wts;
  wts_sum := &+[wts[i] : i in [1..#wts]];
  lambda := &*[all_coeffs[nonzero_inds[i]]^wts[i] : i in [1..#nonzero_inds]];
  // FIXME: something like code below is need to deal with cases where the valuation -1 appears.
  // Otherwise wts are [-1,0,0,...,0] and rescaling doesn't work
