...
intrinsic NewtonGetRescalingEquation(Gamma::GrpPSL2) -> GrpPSL2
  ...
  // setup
    ...
    num_vars := Gamma`TriangleNewtonVariablesNumeratorCoefficients;
    den_vars := Gamma`TriangleNewtonVariablesDenominatorCoefficients;
    c4c6 := Gamma`TriangleNewtonVariablesC4C6;
  ...
  // rescaling
    ...
    // assert wts_sum eq 0;
    curve_and_map_vars := c4c6 cat [R!-1, R!1] cat num_vars cat [R!1] cat den_vars cat [R!1];
    for i := 1 to #wts do
      rescaling_equation *:= Rfrac!(curve_and_map_vars[nonzero_inds[i]])^wts[i];
