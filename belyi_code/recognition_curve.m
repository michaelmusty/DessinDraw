intrinsic TriangleRescaleCoefficients(Gamma::GrpPSL2, coeffs::SeqEnum, vals::SeqEnum : rescale_ind := 0) -> Any
  ...
  */
  ...
  nonzero_inds := [];
  nonzero_vals := [];
  all_coeffs := curve_coeffs cat num_coeffs cat denom_coeffs;
  printf "curve_vals = %o, num_vals = %o, denom_vals = %o\n", curve_vals, num_vals, denom_vals;
  all_vals := curve_vals cat num_vals cat denom_vals;
  ...
  */
  ...
  /*
  if Genus(Gamma) eq 1 then
    curve_coeffs := curve_coeffs cat [-1,1];
  end if;
  */
...
intrinsic TriangleMakeBelyiMap(Gamma::GrpPSL2) -> Any
  ...
  if genus eq 1 then
    ...
    // make Belyi map
    KX<x,y> := FunctionField(X);
    t0 := Gamma`TriangleRiemannRochParameters[1];
    num_basis := RiemannRochBasisHyperellipticExact(t0,X);
    denom_basis := num_basis;
    ...
    // sane := BelyiMapSanityCheck(Gamma`TriangleSigma, X, phi);
    Gamma`TriangleBelyiMap := phi; // before error for debugging
    sane := BelyiLocalSanityCheck(Gamma, 101);
    if not sane then
      print X, phi;
      error "FAILED SANITY CHECK!";
    end if;
