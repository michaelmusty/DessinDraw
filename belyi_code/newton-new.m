...
intrinsic NewtonGetBasicEquations(Gamma::GrpPSL2) -> GrpPSL2
  ...
  // generate polynomial ring
    ...
    // make special point variables if we need to (if not totally ramified)
    if s lt d then
...
intrinsic NewtonGetBasicInitializationValues(Gamma::GrpPSL2) -> GrpPSL2
 ...
 // special points
  ...
  Remove(~den, #den);
  //if s lt d then // if special vars
  if assigned Gamma`TriangleNewtonVariablesSpecial then
    start := curve_coeffs cat white cat black cat cross cat [lc] cat num cat den cat [xs, ys];
  else
